...
category: "技术"
summary: "ASR 用控制论闭环把弱模型堆成强系统：用节奏（Cadence）、闸门（Gate）、反馈（Feedback）三个机制，实现稳定的软件工程输出。"
faq:
  - q: "什么是 Loop Engineering？"
    a: "不再手写提示词，而是编写循环：由循环负责提示模型、决定下一步，并用节奏、闸门、反馈三个机制让模型持续收敛。"
  - q: "ASR 和 Loop Engineering 是什么关系？"
    a: "ASR 基于控制论闭环，用节奏、闸门、反馈三要素落地 Loop Engineering，并在弱模型工程化、DAG 任务调度、Hash-anchored Patch 等方面做了延伸。"
  - q: "ASR 的核心价值是什么？"
    a: "当只有弱模型可用时，也能通过闭环约束获得稳定、可观测、可收敛的软件工程输出。"
---
