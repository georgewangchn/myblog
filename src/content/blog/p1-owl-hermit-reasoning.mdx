...
category: "技术"
summary: "P1 用 OWL 本体 + SWRL 规则 + HermiT 推理机，把宠物疾病诊断编码成形式化逻辑，由推理机自动分类得出结论。"
faq:
  - q: "P1 用的是什么推理技术？"
    a: "OWL 本体建模疾病与症状，SWRL 规则表达诊断逻辑，HermiT 推理机执行自动分类，属于开放世界假设（OWA）下的描述逻辑推理。"
  - q: "为什么用 OWL 而不直接用 if-else？"
    a: "OWL 把诊断知识交给形式化公理和推理机，规则可验证、可解释、可复用，而不是散落在代码里的硬编码条件。"
  - q: "OWL 推理的主要限制是什么？"
    a: "开放世界假设下「不知道」不等于「不存在」，且难以表达递归与传递闭包，这正是后续 P2/P3 要解决的问题。"
---
...
**项目链接**：[github.com/georgewangchn/OntologyOps/tree/main/labs/P1](https://github.com/georgewangchn/OntologyOps/tree/main/labs/P1)
